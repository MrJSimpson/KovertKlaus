...
* **Database:** PostgreSQL 16 Alpine running in Docker container (`kovertklaus-db`) on port 5432.
* **ORM:** Prisma ORM.
  * **Rule:** Whenever modifying `prisma/schema.prisma`, execute `npx prisma db push` to keep PostgreSQL synchronized.

---

## 🎲 5. Secret Target Assignment Derangement Algorithm

* **Secret Santa Operations:** Target assignment MUST use Sattolo's algorithm derangement (`src/lib/draw.ts`). No `Agent` can be assigned to themselves ($A[i] \neq i$), and cycles must be completely deranged.
* **White Elephant Operations:** Do **NOT** generate 1-to-1 targets! Gifting occurs live during the exchange event.

---

## ✅ 6. AI Agent Code Quality & Verification Protocol
...
2. **No Symptom Masking:** Never wrap failing code in silent `try/catch` blocks or comment out broken assertions. Resolve root causes empirically based on build/runtime logs.
3. **Documentation Integrity:** Preserve existing code comments, docstrings, and function signatures.
