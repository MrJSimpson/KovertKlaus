...
export default function LifecycleTestHarness() {
  // Theme state
  const [isDarkMode, setIsDarkMode] = useState<boolean>(true);

  // Virtual Date Simulation Engine (Default: Aug 11, 2026)
  ...
  const [eventLogs, setEventLogs] = useState<string[]>([
    'SYSTEM: Initialized Lifecycle Simulation Harness.',
  ...
  // Days Calculation relative to Virtual Date
  ...
  // Advance Phase Invariant Handler
  const handleAdvancePhase = (nextPhase: OperationPhase) => {
    setPhase(nextPhase);
    // Phase Advancement Date Invariant: set target date to virtualDate
  ...
  return (
    <div className={`min-h-screen pb-16 px-4 sm:px-8 max-w-7xl mx-auto ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-stone-100 text-slate-900'}`}>
      {/* Header & Controls */}
      <header className="py-8 border-b border-gray-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs px-2 py-0.5 rounded font-mono uppercase bg-amber-950/80 text-amber-300 border border-amber-500/30">
              TEST HARNESS // SCHEDULE & STAGE ENGINE
            </span>
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight mt-2 flex items-center gap-3">
            <span>KOVERT KLAUS</span>
            <span className="text-amber-400 font-mono text-sm border border-amber-500/30 px-2 py-1 rounded bg-amber-950/40">
              LIFECYCLE BENCH
            </span>
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="bg-gray-900 hover:bg-gray-800 text-amber-300 border border-amber-500/30 px-3 py-2 rounded-lg font-mono text-xs transition-colors"
          >
            {isDarkMode ? '🎄 Light Theme' : '❄️ Dark Theme'}
          </button>
          <Link
            href="/tests"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          {/* Virtual Clock Panel */}
          <div className={`p-6 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-amber-500/40' : 'bg-white border-amber-400/40'}`}>
            <h2 className="text-sm font-mono font-bold uppercase tracking-wider text-amber-400 mb-3 flex items-center gap-2">
              ...
              <div className="space-y-2 pt-2 border-t border-gray-800">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="bg-slate-800 hover:bg-slate-700 text-sky-300 px-2 py-1.5 rounded text-[11px] text-left border border-slate-700"
                  >
                    RSVP Cutoff (Nov 20)
                  </button>
                  <button
                    onClick={() => { setVirtualDate('2026-11-25'); addLog('PRESET: Jumped to Draw Day (Nov 25)'); }}
                    className="bg-slate-800 hover:bg-slate-700 text-amber-300 px-2 py-1.5 rounded text-[11px] text-left border border-slate-700"
                  >
                    Draw Day (Nov 25)
                  </button>
                  <button
                    onClick={() => { setVirtualDate('2026-12-10'); addLog('PRESET: Jumped to Shipping Deadline (Dec 10)'); }}
                    className="bg-slate-800 hover:bg-slate-700 text-purple-300 px-2 py-1.5 rounded text-[11px] text-left border border-slate-700"
                  >
                    Ship Deadline (Dec 10)
                  </button>
                  <button
                    onClick={() => { setVirtualDate('2026-12-25'); addLog('PRESET: Jumped to Exchange Day (Dec 25)'); }}
                    className="bg-slate-800 hover:bg-slate-700 text-emerald-300 px-2 py-1.5 rounded text-[11px] text-left border border-slate-700"
                  >
                    Exchange Day! (Dec 25)
                  </button>
                </div>
          ...
          <div className={`p-6 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-stone-200'}`}>
            <h2 className="text-sm font-mono font-bold uppercase tracking-wider text-slate-300 mb-4">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={`p-6 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-stone-200'}`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
                <div className="text-xs font-mono text-slate-400 uppercase tracking-widest">ACTIVE STAGE PREVIEW</div>
                <h2 className="text-2xl font-black text-white mt-1">{opName}</h2>
              </div>
          ...
          <div className={`p-6 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-amber-500/30' : 'bg-white border-stone-200'}`}>
          ...
          <div className={`p-6 rounded-xl border-2 ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-stone-200'}`}>
            <h3 className="text-sm font-mono font-bold text-slate-200 uppercase tracking-wider mb-3 flex items-center justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              {phase === 'RECRUITING' && (
                <>
                  <button onClick={() => addLog('DISPATCH: Opened Invite Agent Modal')} className="bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    ➕ Invite Agent
                  </button>
                  <button onClick={() => handleAdvancePhase('SETUP')} className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    🔒 Close Recruitment
                  </button>
                </>
              )}
              ...
              {phase === 'SETUP' && (
                <>
                  <button onClick={() => addLog('DISPATCH: Opened Emergency Invite Modal')} className="bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    ⚡ Emergency Invite
                  </button>
                  <button onClick={() => addLog('DISPATCH: Opened Bidirectional Matching Rules Modal')} className="bg-purple-500 hover:bg-purple-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    🚫 Matching Rules ($A \iff B$)
                  </button>
                  <button onClick={() => handleAdvancePhase('ASSIGNED')} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
              ...
              {(phase === 'ASSIGNED' || phase === 'EXECUTED') && (
                <>
                  <button onClick={() => addLog('DISPATCH: Opened OpTeam Broadcast Alert Modal')} className="bg-sky-500 hover:bg-sky-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    📢 Send OpTeam Broadcast
                  </button>
                  <button onClick={() => addLog('DISPATCH: Opened Target Swap Dropdown Modal')} className="bg-purple-500 hover:bg-purple-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    🎯 Target Swap Console
                  </button>
                  <button onClick={() => handleAdvancePhase('COMPLETED')} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    🏁 End Operation
                  </button>
                </>
              )}
              ...
              {phase === 'COMPLETED' && (
                <>
                  <button onClick={() => addLog('DISPATCH: Opened Demerit Citation Console')} className="bg-rose-500 hover:bg-rose-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
                    ⚠️ Issue Demerit Citation
                  </button>
                  <button onClick={() => addLog('DISPATCH: Opened AAR Debrief Photo Upload')} className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold px-4 py-2 rounded-xl text-xs shadow-md">
          ...
          <div className="bg-slate-950 border border-gray-800 rounded-xl p-4 font-mono text-xs">
            <div className="flex items-center justify-between text-gray-400 border-b border-gray-800 pb-2 mb-3">
              ...
              <button onClick={() => setEventLogs([])} className="text-[11px] text-gray-500 hover:text-red-400">Clear Terminal</button>
            </div>
