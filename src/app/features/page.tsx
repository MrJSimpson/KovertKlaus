...
export default function FeaturesPage() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {/* Header Bar */}
      <header className={`border-b transition-colors ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-5 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center font-extrabold text-white text-xl shadow-md">
              K
            </div>
            <div>
              ...
              <span className="text-xs text-slate-500 block font-mono">Features & Engineering Specs</span>
            </div>
          </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-16">
        ...
        <div className="max-w-3xl mb-16">
          <span className="text-xs font-mono font-bold text-emerald-600 uppercase tracking-widest block mb-2">SYSTEM ARCHITECTURE & FEATURES</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Sattolo Algorithm */}
          <div className={`p-8 rounded-2xl border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-md'}`}>
            <div className="h-12 w-12 rounded-xl bg-emerald-100 text-emerald-700 flex items-center justify-center text-2xl font-bold mb-6">
              🔗
            </div>
            ...
            <ul className={`text-xs space-y-2 font-mono ${isDarkMode ? 'text-emerald-400' : 'text-emerald-800'}`}>
          ...
          <div className={`p-8 rounded-2xl border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-md'}`}>
            <div className="h-12 w-12 rounded-xl bg-amber-100 text-amber-700 flex items-center justify-center text-2xl font-bold mb-6">
              🛡️
            </div>
            <h2 className={`text-2xl font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
            ...
            <ul className={`text-xs space-y-2 font-mono ${isDarkMode ? 'text-slate-300' : 'text-slate-700'}`}>
              ...
              <li className="text-emerald-600 font-semibold pt-1">
          ...
          <div className={`p-8 rounded-2xl border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-md'}`}>
            <div className="h-12 w-12 rounded-xl bg-teal-100 text-teal-700 flex items-center justify-center text-2xl font-bold mb-6">
              ⚡
            </div>
            <h2 className={`text-2xl font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
          ...
          <div className={`p-8 rounded-2xl border transition-all ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-md'}`}>
            <div className="h-12 w-12 rounded-xl bg-rose-100 text-rose-700 flex items-center justify-center text-2xl font-bold mb-6">
              🎁
            </div>
            <h2 className={`text-2xl font-bold mb-3 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
            ...
            <p className={`text-sm leading-relaxed mb-4 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
              White Elephant and Yankee Swap exchanges are strictly restricted to local, in-person events (<code className="font-mono text-xs text-rose-600 font-bold">isLocalOnly = true</code>). Digital White Elephant swaps are disabled to preserve authentic community interaction and stealing dynamics.
            </p>
          </div>
        ...
        {/* License Banner */}
        <div className={`mt-16 p-8 rounded-2xl border ${isDarkMode ? 'bg-slate-900 border-emerald-500/30' : 'bg-emerald-50/80 border-emerald-200'}`}>
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            <div>
              <span className="text-xs font-mono font-bold text-emerald-700 uppercase tracking-widest block mb-1">BUSINESS SOURCE LICENSE 1.1</span>
            ...
            <Link
              href="/tests"
              className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-sm px-5 py-3 rounded-xl transition-all shadow-md shrink-0"
            >
